    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6 w-full max-w-9xl mx-auto">
      ...
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-1">
          <Link href="/dashboard/technadzor/control-points" title={t('Отмена')} className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700/40 transition">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </Link>
          <button onClick={() => save('draft')} disabled={saving} title={t('Сохранить черновик')} className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700/40 transition disabled:opacity-50">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
            </svg>
          </button>
          {step < STEPS.length ? (
            <button onClick={() => save('draft', true)} disabled={saving} title={t('Сохранить и далее')} className="p-2 rounded-lg text-violet-600 dark:text-violet-300 hover:bg-violet-50 dark:hover:bg-violet-500/10 transition disabled:opacity-50">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
              </svg>
            </button>
          ) : (
            <button onClick={() => save('active')} disabled={saving} title={t('Опубликовать пункт')} className="p-2 rounded-lg text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-500/10 transition disabled:opacity-50">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <Card title="Совместимость с инспекциями">
              <div className="space-y-2">
                {COMPAT_OPTIONS.map((c) => {
                  ...
                  return (
                    <label key={c} className="flex items-center gap-2.5 text-sm text-gray-700 dark:text-gray-200 cursor-pointer select-none">
                      <input type="checkbox" checked={on} onChange={(e) => setCompat((cs) => e.target.checked ? [...cs, c] : cs.filter((x) => x !== c))} className="peer sr-only" />
                      <span className="w-5 h-5 shrink-0 rounded-md border border-gray-300 dark:border-gray-600 flex items-center justify-center transition-colors peer-checked:bg-violet-600 peer-checked:border-violet-600">
                        <svg className={`w-3.5 h-3.5 text-white transition-opacity ${on ? 'opacity-100' : 'opacity-0'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
