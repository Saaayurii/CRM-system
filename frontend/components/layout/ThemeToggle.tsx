...
export default function ThemeToggle({ navItem }: { navItem?: boolean }) {
  const t = useT();
  ...
  if (navItem) {
    return (
      <div>
        <input type="checkbox" name="light-switch" id="light-switch" className="light-switch sr-only"
          checked={theme === 'light'} onChange={toggleTheme} suppressHydrationWarning />
        <label htmlFor="light-switch"
          className="flex items-center gap-3 py-2 px-3 rounded-lg w-full cursor-pointer text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition duration-150 truncate">
          <svg className="w-5 h-5 shrink-0 dark:hidden fill-current" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
          ...
          <svg className="w-5 h-5 shrink-0 hidden dark:block fill-current" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
          ...
          <span className="text-sm font-medium lg:opacity-0 lg:sidebar-expanded:opacity-100 duration-200">{t('Тема')}</span>
        </label>
      </div>
    );
  }
