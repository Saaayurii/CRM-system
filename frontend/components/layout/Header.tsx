...
function CompanyName() {
  ...
  const displayName = isGlobal && activeAccount
    ? activeAccount.name
    ...
    <div className="relative" ref={ref}>
      ...
      >
        {activeAccount?.logoUrl && (
          <img src={activeAccount.logoUrl} alt="" className="w-5 h-5 rounded object-cover shrink-0" />
        )}
        <span className="text-sm font-medium text-gray-700 dark:text-gray-200 truncate max-w-[160px]">
          {selectedAccountId ? (activeAccount?.name ?? 'Компания') : 'Все компании'}
        </span>
