...
/** Дефолтный ключ настроек отображения общей страницы задач. */
export const TASKS_DISPLAY_SETTINGS_KEY = 'tasksDisplaySettings';

function loadDisplaySettings(key: string): DisplaySettings {
...
async function fetchTasksPageData(projectId?: number): Promise<TasksPageData> {
  const [tasksRes, projectsRes, usersRes] = await Promise.all([
    api.get('/tasks', { params: projectId ? { projectId, limit: 500 } : { limit: 500 } }),
    projectId ? Promise.resolve({ data: {} as any }) : api.get('/projects', { params: { limit: 100 } }),
...
function TableSettingsModal({ settings, onChange, onClose, hiddenKeys }: {
  settings: DisplaySettings;
  onChange: (s: DisplaySettings) => void;
  onClose: () => void;
  hiddenKeys?: (keyof DisplaySettings)[];
}) {
  const t = useT();
    ...
    >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          {DISPLAY_SECTIONS.map((section) => {
            const items = section.items.filter((item) => !hiddenKeys?.includes(item.key));
...
export interface TasksViewProps {
  /** Если задан — список привязан к проекту: фильтр+колонка «Проект» скрыты,
   *  создание задачи лочит проект, fetch идёт только по этому проекту. */
  projectId?: number;
  title?: string;
  subtitle?: string;
  /** Ключ offline-кэша; по умолчанию tasks-page / tasks-project-<id>. */
  cacheKey?: string;
  /** Ключ localStorage настроек колонок (раздельно по контексту). */
  settingsStorageKey?: string;
  /** Ключ localStorage режима table/grid. */
...
export default function TasksView({
  ...
  subtitle,
  cacheKey,
  settingsStorageKey = TASKS_DISPLAY_SETTINGS_KEY,
  viewStorageKey = 'dashViewMode',
}: TasksViewProps) {
  const t = useT();
  ...
  const [viewMode, setViewMode] = useState<'table' | 'grid'>(() => {
    if (typeof window !== 'undefined') return (localStorage.getItem(viewStorageKey) as 'table' | 'grid') || 'table';
    return 'table';
  });
  ...
  const [displaySettings, setDisplaySettings] = useState<DisplaySettings>(() => loadDisplaySettings(settingsStorageKey));
  const [showTableSettings, setShowTableSettings] = useState(false);

  useEffect(() => {
    try { localStorage.setItem(settingsStorageKey, JSON.stringify(displaySettings)); } catch {}
  }, [displaySettings, settingsStorageKey]);

  useEffect(() => { if (!projectId) markTasksRead(); }, [projectId]);
  ...
  const effectiveCacheKey = cacheKey ?? (projectId ? `tasks-project-${projectId}` : 'tasks-page');
  const { data, loading, error, refetch } =
    useOfflineData<TasksPageData>(() => fetchTasksPageData(projectId), effectiveCacheKey);
  ...
  useEffect(() => {
    if (projectId) return; // deep-link ?edit= — только на общей странице задач
  ...
  const handleCreate = (pid?: number) => {
    setQuickCreateProjectId(pid ?? projectId);
    setEditingTask(null);
    setShowModal(true);
  };
  ...
  const showProjectCol = !projectId && ds.colProject;

  const COLUMNS: { key: SortKey | null; label: string }[] = [
    { key: 'title', label: 'Название' },
    ...(showProjectCol   ? [{ key: 'project' as SortKey,   label: 'Проект' }] : []),
  ...
  return (
    <div>
      <div className="sm:flex sm:items-center sm:justify-between mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">{title ?? t('Задачи')}</h1>
          {(subtitle ?? t('Управление задачами проекта')) && (
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{subtitle ?? t('Управление задачами проекта')}</p>
          )}
        </div>
        <div className="relative flex items-center gap-2 mt-3 sm:mt-0" ref={settingsRef}>
          ...
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 p-1 rounded-lg">
            <button
              onClick={() => { setViewMode('table'); localStorage.setItem(viewStorageKey, 'table'); }}
            ...
            <button
              onClick={() => { setViewMode('grid'); localStorage.setItem(viewStorageKey, 'grid'); }}
      ...
      {showFilter && <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {/* Project filter — скрыт в режиме привязки к проекту */}
        {!projectId && (
          <select
            value={filterProject ?? ''}
            ...
            return (
              <div key={task.id} className={`group/card border rounded-xl p-4 flex flex-col gap-3 hover:shadow-md transition-shadow ${rowOverdue ? 'bg-red-50/70 dark:bg-red-900/10 border-red-300 dark:border-red-700/60' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'}`}>
                ...
                <dl className="grid grid-cols-2 gap-x-3 gap-y-2">
                  {showProjectCol && <div><dt className="text-xs text-gray-400">{t('Проект')}</dt><dd className="text-xs text-gray-700 dark:text-gray-300 truncate">{task.project?.name || '—'}</dd></div>}
      ...
      {historyTooltip && (
        ...
        >
          ...
          <button
            onClick={() => { const tk = tasks.find((tk) => tk.id === historyTooltip.taskId); if (tk) openTaskHistory(tk); setHistoryTooltip(null); }}
      ...
      {showTableSettings && (
        <TableSettingsModal
          ...
          hiddenKeys={projectId ? ['colProject'] : undefined}
        />
      )}
      ...
      {showModal && (
        <TaskFormModal
          task={editingTask}
          initialProjectId={!editingTask ? (quickCreateProjectId ?? projectId) : undefined}
          lockProjectId={!editingTask && !!projectId}
          onClose={() => {
            setShowModal(false);
            setEditingTask(null);
            setQuickCreateProjectId(undefined);
