...
export class DashboardWidgetRepository {
  ...
  async findAll(userId: number, page: number = 1, limit: number = 20) {
    ...
    const [data, total] = await Promise.all([
      (this.prisma as any).dashboardWidget.findMany({
        where: { userId },
        skip,
        take: limit,
  ...
  async findById(id: number, userId: number) {
    return (this.prisma as any).dashboardWidget.findFirst({
      where: { id, userId },
    });
  }
  async create(userId: number, dto: CreateDashboardWidgetDto) {
    return (this.prisma as any).dashboardWidget.create({
      data: { ...dto, userId },
    });
  }
  async update(id: number, userId: number, dto: UpdateDashboardWidgetDto) {
    await (this.prisma as any).dashboardWidget.updateMany({
      where: { id, userId },
      data: { ...dto },
    });
    return (this.prisma as any).dashboardWidget.findFirst({
  ...
  async delete(id: number, userId: number) {
    return (this.prisma as any).dashboardWidget.deleteMany({
      where: { id, userId },
    });
  }
}
