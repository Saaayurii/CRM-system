...
export class ExecutionLogService {
  ...
  async findAll(page: number, limit: number, automationRuleId?: number) {
    return this.repo.findAll(page, limit, automationRuleId);
  }
  async findById(id: number) {
    const l = await this.repo.findById(id);
    if (!l) throw new NotFoundException(`Execution log #${id} not found`);
    return l;
  }
