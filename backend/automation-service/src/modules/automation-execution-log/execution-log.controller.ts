...
@ApiTags('Automation Execution Log')
@ApiBearerAuth()
@Controller('automation-execution-log')
export class ExecutionLogController {
  ...
  @ApiQuery({ name: 'automationRuleId', required: false })
  findAll(
    @Query('page') page: number = 1,
    @Query('limit') limit: number = 20,
    @Query('automationRuleId') ruleId?: number,
  ) {
  ...
  create(@Body() dto: CreateExecutionLogDto) {
