...
export class KnowledgeTestsController {
  ...
  @Post() @ApiOperation({ summary: 'Create knowledge test' }) create(
    @Body() dto: CreateKnowledgeTestDto,
    @CurrentUser('accountId') accountId: number,
  ) {
    return this.svc.create(accountId, dto);
  }
  @Put(':id') @ApiOperation({ summary: 'Update knowledge test' }) update(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: UpdateKnowledgeTestDto,
    @CurrentUser('accountId') accountId: number,
  ) {
    return this.svc.update(id, accountId, dto);
  }
  @Delete(':id') @ApiOperation({ summary: 'Delete knowledge test' }) remove(
    @Param('id', ParseIntPipe) id: number,
    @CurrentUser('accountId') accountId: number,
  ) {
