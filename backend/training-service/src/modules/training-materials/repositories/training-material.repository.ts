...
export class TrainingMaterialRepository {
  ...
  async findAll(
    ...
    const [data, total] = await Promise.all([
      (this.prisma as any).trainingMaterial.findMany({
        where,
        skip,
        take: limit,
  ...
  async findById(id: number, accountId: number) {
    return (this.prisma as any).trainingMaterial.findFirst({
      where: { id, accountId },
    });
  }
  async create(accountId: number, dto: CreateTrainingMaterialDto) {
    return (this.prisma as any).trainingMaterial.create({
      data: { ...dto, accountId },
    });
  }
  async update(id: number, accountId: number, dto: UpdateTrainingMaterialDto) {
    await (this.prisma as any).trainingMaterial.updateMany({
      where: { id, accountId },
      data: { ...dto },
    });
    return (this.prisma as any).trainingMaterial.findFirst({
  ...
  async delete(id: number, accountId: number) {
    return (this.prisma as any).trainingMaterial.deleteMany({
      where: { id, accountId },
    });
  }
}
