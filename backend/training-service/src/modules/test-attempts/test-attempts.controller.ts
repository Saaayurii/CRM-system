...
export class TestAttemptsController {
  ...
  @ApiQuery({ name: 'knowledgeTestId', required: false })
  @ApiQuery({ name: 'userId', required: false })
  findAll(
    @Query('page') page: number = 1,
    @Query('limit') limit: number = 20,
    @Query('knowledgeTestId') ktId?: number,
  ...
  @Get(':id') @ApiOperation({ summary: 'Get test attempt by ID' }) findOne(
    @Param('id', ParseIntPipe) id: number,
  ) {
    return this.svc.findById(id);
  }
  @Post() @ApiOperation({ summary: 'Create test attempt' }) create(
    @Body() dto: CreateTestAttemptDto,
  ) {
