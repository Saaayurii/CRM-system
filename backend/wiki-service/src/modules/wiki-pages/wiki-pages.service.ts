...
export class WikiPagesService {
  ...
  async findAll(accountId: number, page: number, limit: number, category?: string, q?: string) {
    return this.wikiPageRepository.findAll(accountId, page, limit, category, q);
  }

  async findTree(accountId: number) {
    return this.wikiPageRepository.findTree(accountId);
  }
  ...
  async getVersions(id: number, accountId: number) {
    await this.findById(id, accountId);
    return this.wikiPageRepository.getVersions(id);
  }

  async getVersion(id: number, versionNum: number, accountId: number) {
    await this.findById(id, accountId);
    return this.wikiPageRepository.getVersion(id, versionNum);
  }

  async incrementView(id: number, accountId: number) {
    await (this.wikiPageRepository as any).prisma.wikiPage.updateMany({
      where: { id, accountId },
