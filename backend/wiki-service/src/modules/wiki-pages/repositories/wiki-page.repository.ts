...
export class WikiPageRepository {
  ...
  async findAll(accountId: number, page: number = 1, limit: number = 20, category?: string) {
    ...
    const [data, total] = await Promise.all([
      (this.prisma as any).wikiPage.findMany({ where, skip, take: limit, orderBy: { updatedAt: 'desc' } }),
  ...
  async create(accountId: number, userId: number, dto: CreateWikiPageDto) {
    return (this.prisma as any).wikiPage.create({ data: { ...dto, accountId, createdByUserId: userId, updatedByUserId: userId } });
  }

  async update(id: number, accountId: number, userId: number, dto: UpdateWikiPageDto) {
    await (this.prisma as any).wikiPage.updateMany({ where: { id, accountId }, data: { ...dto, updatedByUserId: userId } });
    return (this.prisma as any).wikiPage.findFirst({ where: { id, accountId } });
  }
